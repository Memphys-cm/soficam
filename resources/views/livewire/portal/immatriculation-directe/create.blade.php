            ...
            <div class="modal-body p-0">
                <div class="p-4 p-lg-5">
                    <div class="mb-4 mt-md-0">
                        <h1 class="mb-0 h4"> {{$state ? __('Modifier') : __('Enregistrer')}}{{__(' une Immatrculation Directe')}}</h1>
                        <p class="px-1"> {{$state ? __('Modifier') : __('Enregistrer')}}{{__(' un Dossier D\'Immatrculation Directe')}} </p>
                    </div>
                    <x-form-items.form wire:submit="{{$state ? 'update':'store'}}">
                        ...
                        <div class='form-group mb-3 row'>
                            <div class="col">
                                ...
                                <input wire:model="imma_directe.localisation" type="text" class="form-control  @error('imma_directe.localisation') is-invalid @enderror" placeholder="{{__('Yaounde , Mendong')}}" required="" name="imma_directe.localisation">
                                @error('imma_directe.localisation')
                                <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                            </div>
                        </div>
                        ...
                        @foreach($comissions as $comissionIndex => $comission)
                        <div class='form-group mb-2 d-flex align-items-end justify-content-between'>
                            <div class=''>
                                <label for="coordonnees.{{$comissionIndex}}.nom">{{__('Nom')}} - B{{ $loop->iteration }}</label>
                                <input wire:model="coordonnees.{{$comissionIndex}}.nom" type="text" step="0.0001" class="form-control col-md-12 @error('coordonnees') is-invalid @enderror" placeholder="{{__('Onana Franck')}}" required="" value="" name="coordonnees">
                                @error('coordonnees')
                                <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                            </div>
                            <div class=''>
                                <label for="coordonnees.{{$comissionIndex}}.poste">{{__('Nom')}} - B{{ $loop->iteration }}</label>
                                <input wire:model="coordonnees.{{$comissionIndex}}.poste" type="text" step="0.0001" class="form-control col-md-12 @error('coordonnees') is-invalid @enderror" placeholder="{{__('Franck')}}" required="" value="" name="coordonnees">
                                @error('coordonnees')
                                <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                            </div>
                            <a type="button" wire:click="removecomission({{ $comissionIndex }})" class="btn-icon ">
                        ...
                        @endforeach
                        <hr>
                        <button type="button" wire:click="addcomission" class="btn btn-sm btn-outline-primary">
