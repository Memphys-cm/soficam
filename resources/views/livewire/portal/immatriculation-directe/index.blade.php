    ...
    <td>
        @can('imma_directe.view_detail')
            <a href="#" data-bs-placement="top" title="Voir Les Deatils du Dossier">
                <svg class="icon icon-sm text-info" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
        ...
        @can('imma_directe.etat_cession', $imma_directe)
            <a href="#" data-bs-placement="top" title="Etat de Cession"
                wire:click.prevent="initData({{ $imma_directe->id }})" data-bs-toggle="modal" data-bs-target="#EtatCessionModal"
                draggable="false">
        ...
        @can('imma_directe.update', $imma_directe)
            <a href="#" data-bs-placement="top" title="Modifier le Staut du Dossier Ici"
                wire:click.prevent="initData({{ $imma_directe->id }})" data-bs-toggle="modal" data-bs-target="#EditStatutModal"
                draggable="false">
                <svg class="icon icon-sm text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @can('imma_directe.ordre_versement', $imma_directe)
            ...
            @if ($imma_directe->statut == 'Ordre de Versement en Attente de Paiement')
                <a href="#" data-bs-placement="top" title="Etablisser L'ordre de Versement"
                    wire:click.prevent='ordreDeVersementPdf({{ $imma_directe->id }})' draggable="false">
                    <svg class="icon icon-sm text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        @can('imma_directe.dossier_vise', $imma_directe)
            @if ($imma_directe->next_step == 'Traitement du dossiser vise-enregistre')
                <a href='#' title="Montant à payer" wire:click.prevent="initData({{ $imma_directe->id }})"
        ...
        @can('imma.directe.convocation', $imma_directe)
            <a href="#" data-bs-placement="top" title="Imprimer La convocation D'Invitation sur le Terrain"
                wire:click.prevent="initData({{ $imma_directe->id }})" data-bs-toggle="modal"
            ...
            @if ($imma_directe->next_step == 'signature decision portant calendrier de descente')
                <a href="#" data-bs-placement="top" title="Imprimer le certifacat d'affichage"
                    wire:click.prevent="initData({{ $imma_directe->id }})" data-bs-toggle="modal"
                    data-bs-target="#ConvocationImmaDirecteModal" draggable="false">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor">
        ...
        @can('imma_directe.enregistrer_geometre', $imma_directe)
            @if ($imma_directe->next_step == 'Etat de cession enregistré auprès du géomètre')
                <a href="#" data-bs-placement="top" title="Enregistrer Le Geometre"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" class="icon icon-xs" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M9 13.5l3 3m0 0l3-3m-3 3v-6m1.06-4.19l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z" />
                    </svg>
        ...
        @can('titre_foncier.delete')
            <a href="#" data-bs-placement="top" title="Supprimer le Dossier Ici"
                wire:click.prevent="initData({{ $imma_directe->id }})" data-bs-toggle="modal" data-bs-target="#DeleteModal"
                href="#" draggable="false">
                <svg class="icon icon-sm text-danger me-2" fill="currentColor" viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
