<div>
    <x-alert />
    @include('livewire.portal.certificat-propriete.demandecp.partials.create-edit')
    ...
    <div class='p-0'>
        <div class="d-flex justify-content-between w-100 flex-wrap align-items-center">
            ...
            <div class="d-flex justify-content-between mb-2">
                ...
                {{-- @can('user.create') --}}
                <a href="#" data-bs-toggle="modal" data-bs-target="#CreatecpdemandeModal"
    ...
    <div class="card pb-3">
        <div class="table-responsive  text-gray-700">
            <table class="table employee-table table-hover align-items-center ">
                <thead>
                    <tr>
                        <th class="border-bottom">{{ __('PURCHASER NAME') }}</th>
                        <th class="border-bottom">{{ __('LAND OWNER') }}</th>
                ...
                <tbody>
                    @forelse($cpdemandes as $cpdemande)
                        <tr>
                            <td>
                                <a href="#" class="d-flex align-items-center">
                                    <div class="avatar d-flex align-items-center justify-content-center fw-bold rounded bg-primary me-3"><span class="text-white">{{initials($cpdemande->purchaser_name )}}</span></div>
                                    <div class="d-block"><span class="fw-bold">{{$cpdemande->purchaser_name }}</span>
                                        <div class="small text-gray">{{!empty($cpdemande->purchaser_name ) ? $cpdemande->purchaser_name  : ''}}</div>
                                    </div>
                                </a>
                            </td>
                            <td>{{ $cpdemande->land_owner }}</td>
                            
                            <td>
                                {{-- <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $this->getStatusClass($cpdemande->status) }}">
                                    {{ ucfirst($cpdemande->status) }}
                                </span> --}}
                            <span class="fw-normal badge super-badge p-2 bg-{{$cpdemande->statusStyle}} round">{{$cpdemande->status}}</span>

                            </td>
                            <td>{{ $cpdemande->created_at }}</td>
                                <td>
                                        <a href='#' wire:click.prevent="initData({{ $cpdemande -> id }})"
                                            data-bs-toggle="modal" data-bs-target="#Editcpdemandedal">
...
@push('scripts')
    <script>
        window.addEventListener('close-modal', event => {
            $('#CreatecpdemandeModal').modal('hide');
            $('#DeleteModal').modal('hide');
            $('#Editcpdemandeodal').modal('hide');
        });
    </script>
@endpush
