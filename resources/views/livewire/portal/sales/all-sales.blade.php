<div>
    ...
    <div class="row p-3">
        ...
        <div class="row">
            ...
            <div class="col-md-3 my-2">
                <label for="service">{{ __('Status') }}: </label>
            ...
            <div class="col-md-3 my-2">
                <label for="inter_start">{{ __('Start Period') }}</label>
                <input wire:model="inter_start" name="inter_start" type="date" class="form-control  @error('inter_start') is-invalid @enderror" required="">
            </div>
            <div class='col-md-3 my-2'>
                <label for="inter_end">{{ __('End Date') }}</label>
                <input wire:model="inter_end" name="inter_end" type="date" class="form-control @error('inter_end') is-invalid @enderror" required="">
            </div>
            <div class="col-md-3">
                <label for="service">{{ __('Service') }}</label>
                <x-input.select wire:model="service" prettyname="service" :options="$services->pluck('service_name_fr', 'id')->toArray()"
                    selected="('service')" />
                @error('service')
            ...
            <div class='col'>
                ...
                <x-input.select wire:model="receveur" prettyname="receveur" :options="$receveurs->pluck( 'first_name','id')->toArray()" />
                @error('user_ids')
                <div class="invalid-feedback">{{$message}}</div>
                @enderror
            </div>
    ...
    <div class="row p-3">
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                ...
                <x-input.select wire:model="notary" prettyname="notary" :options="$notaries->pluck('name', 'id')->toArray()"
                    selected="('notary')" />
                @error('notary')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
        ...
        <div class="table-responsive  text-gray-700">
            <table class="table employee-table table-bordered table-hover align-items-center ">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <span class="fw-normal">{{$allsale->user_id}}</span>
                        </td>
                        <td>
                            <span class="fw-normal">{{$allsale->sales_amount}}</span>
                        </td>
                        <td>
                            <span class="fw-normal">{{$allsale->service_id}}</span>
                        </td>
                        <td>
                            <span class="fw-normal">{{$allsale->payment_method}}</span>
                        </td>
                        <td>
                            
                            <span class="fw-normal badge super-badge p-2 bg-{{$allsale->statusStyle}} round">{{$allsale->status}}</span>

                        </td>

                        <td>
                            <span class="fw-normal">{{$allsale->created_at->format('Y-m-d')}}</span>
                        </td>
            ...
            <div class='d-flex justify-content-between align-items-center pt-3 px-3 '>
                <div>
                    {{__('Showing')}} {{$perPage > $allsales_count ? $allsales_count : $perPage  }} {{__('items of')}} {{$allsales_count}}
                </div>
                {{ $allsales->links() }}
            </div>
        </div>
