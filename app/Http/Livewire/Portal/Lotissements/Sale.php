...
class Sale extends Component
{
    ...
    public $users, $notares, $surface_for_sale, $price_per_m_square, $sale_amount, $payment_method, $notary, $service_id;
    public $notaires, $lotissement, $surperficie_du_lot, $notaire_id;
    public $user_ids = [];
    public $montant_versee, $montant_restant, $type_de_versement, $commentaire_du_notaire;
    public string $superficie_a_vendre = 'total';
    public int $superficie_vendu = 0;
    public int $superficie_restant = 0;
    ...
    public function initLotData($id)
    {
        ...
        $this->surperficie_du_lot = $this->parcel->surperficie_du_lot;
    }

    public function calculateSuperficieRestant()
    {
        if ($this->superficie_a_vendre === 'partielle' && !is_null($this->superficie_vendu)) {
            if ($this->superficie_vendu <= $this->surperficie_du_lot) {
                $this->superficie_restant = intval($this->surperficie_du_lot) - intval($this->superficie_vendu);
            } else {
                // Display an error message
                $this->addError('superficie_vendu', 'The value entered is more than Total Area.');
                $this->superficie_restant = 0;
            }
        } else {
            $this->superficie_restant = 0;
        }
    ...
    {
        if (!empty($this->surperficie_du_lot) && !empty($this->price_per_m_square)) {
            if ($this->superficie_a_vendre === 'total') {
                $this->sale_amount = intval($this->surperficie_du_lot) * intval($this->price_per_m_square);
                // Calculate the sale amount based on superficie_vendu and price_per_m_square
            } elseif ($this->superficie_a_vendre === 'partielle') {
                // Calculate the sale amount based on surperficie_du_lot and price_per_m_square
                $this->sale_amount = intval($this->superficie_vendu) * intval($this->price_per_m_square);
            }

            if ($this->type_de_versement === 'tranche') {
                if ($this->montant_versee) {
                    $this->montant_restant = $this->sale_amount - intval($this->montant_versee);
                } else {
    ...
    public function updatedPricePerMSquare()
    {
        // $this->calculateSaleAmount();
        $this->sale_amount = intval($this->surperficie_du_lot) * intval($this->price_per_m_square);
    }
